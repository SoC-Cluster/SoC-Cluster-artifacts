...
if [ "$#" -ne 3 ]; then
	...
	echo "Usage: run_mnn.sh <run time for 1 ite> <ites> <output:filename>"
	echo "Example: run_mnn.sh 4 10 "
    exit
fi
...
command="/data/local/tmp/benchmark.out /data/local/tmp/MNN_Models 0 0 7 8"
cnt=0
while [ $cnt -lt $ites ]
do
    echo $cnt
